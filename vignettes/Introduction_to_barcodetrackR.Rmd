...
Here, we view the correlation of barcode _"percentages"_ between different cell types at the 20 month timepoint of the Wu et al study. We compare granulocytes (Gr) to B and T cells.

```{r scatter_plot, eval = TRUE, fig.width = 7, fig.height=4}
Gr_B_20 <- c("ZJ31_20m_Gr", "ZJ31_20m_B")
Gr_T_20 <- c("ZJ31_20m_Gr", "ZJ31_20m_T")
wu_scatterplot_1 <- scatter_plot(wu_SE[,Gr_B_20], your_title = "Gr vs B", assay = "percentages")
wu_scatterplot_2 <- scatter_plot(wu_SE[,Gr_T_20], your_title = "Gr vs T", assay = "percentages")
cowplot::plot_grid(wu_scatterplot_1, wu_scatterplot_2, ncol = 2)
```

## `cor_plot`
A more comprehensive way to view the relationship between samples in a pairwise manner is to use a correlation plot. Here, we visualize the Pearson correlation of the barcode _"percentages"_ between the T, B, Gr, NK CD56+/CD16-, and NK CD56-/CD16+ fractions within the Wu dataset for the 6, 9.5, 12, and 20 month post-transplant timepoints.
...
```{r dist plot 1, eval = TRUE, fig.width = 6, fig.height = 5}
...
```{r dist plot 2, eval = TRUE, fig.width = 8, fig.height = 5}
dist_plot(wu_SE[,wu_cor_plot_sample_selection],
          dist_method =  "cosine",
          plot_type = "color",
          assay = "logs",
...
The Belderbos dataset generally contains less clones than either the Wu or Six datasets, so by using the "percentages" option of the `cellnote_assay` we can see the actual percentages of each sample.
...
```{r bias_ridge_plot wu_SE 1, echo=TRUE, warning = FALSE, eval = TRUE}
...
```{r bias_ridge_plot wu_SE 2, echo=TRUE, warning = FALSE, eval = TRUE}
bias_ridge_plot(your_SE = wu_SE,
                split_bias_on = "celltype",
                bias_1 = "B",
                bias_2 = "Gr",
...
mds_plot(wu_SE, group_by = "celltype", method_dist = "bray", assay = "percentages")
```
...
mds_plot(six_SE, group_by = "celltype", method_dist = "bray", assay = "percentages")
```
