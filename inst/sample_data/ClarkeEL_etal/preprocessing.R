...
load('your_file_path/data/tcr.rda') # Downloaded from zenodo link above
...
# Write to files
write.table(file = "inst/sample_data/ClarkeEL_etal/tcr_reads_p00007.txt", clarke_TCR_reads_p7, sep = '\t', quote = FALSE, row.names = T, col.names  = NA)
write.table(file = "inst/sample_data/ClarkeEL_etal/tcr_metadata_p00007.txt", clarke_TCR_metadata_p7, sep = '\t', quote =  FALSE, row.names = F)
...
colnames(raw_clarke_data_reads) <- paste0("timepoint_",colnames(raw_clarke_data_reads), "_months")
...
colnames(raw_clarke_data_cells) <- paste0("timepoint_",colnames(raw_clarke_data_cells), "_months")
raw_clarke_data_cells[is.na(raw_clarke_data_cells)] <- 0

p00007.metadata <- data.frame(SAMPLENAME = c("timepoint_4_months","timepoint_6_months", "timepoint_12_months", "timepoint_18_months", "timepoint_24_months"),
                              months = c(4, 6, 12, 18, 24))

all(rownames(raw_clarke_data_cells) == rownames(raw_clarke_data_reads))

write.table(file = "inst/sample_data/ClarkeEL_etal/p00007_reads.txt", raw_clarke_data_reads, sep = '\t', quote = FALSE, row.names = T, col.names  = NA)
write.table(file = "inst/sample_data/ClarkeEL_etal/p00007_cells.txt", raw_clarke_data_cells, sep = '\t', quote = FALSE, row.names = T, col.names  = NA)
write.table(file = "inst/sample_data/ClarkeEL_etal/p00007_metadata.txt", p00007.metadata, sep = '\t', quote =  FALSE, row.names = F)
