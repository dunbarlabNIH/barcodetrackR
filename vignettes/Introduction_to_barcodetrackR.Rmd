...
Similarly, for the Six, et al data.
...
The ridge plot function calculates the density of clones at different values of bias between two selections. Note that log_bias is calculated as:

$logbias=log(\frac{count_1+1}{count_2+1})$

so that it can handle clones which are zero in one of the samples.

Here, we view a ridge plot showing clonal bias between B and T cells from the Wu dataset.

```{r ridge_plot wu_SE, echo=TRUE, warning = FALSE, eval = TRUE}
wu_ridge_plot <- ridge_plot(your_SE = wu_SE[,wu_bias_plot_sample_selection],
                            split_bias_on = "celltype",
                            bias_1 = "B",
                            bias_2 = "T",
                            split_bias_over = "months")
...
bias_distribution_barplot(your_SE = wu_SE[,wu_ridge_plot_sample_selection],
               split_bias_on = "celltype",
               bias_1 = "B",
               split_bias_over = "months")
```

Right now, error in line 52. Will come back to this.

## `ternary_plot`

A ternary plot can be made for any SE with just 3 samples in it. 

```{r ternary_plot wu_SE, echo=TRUE, warning = FALSE, eval = TRUE}
...
For now, I just quickly changed the current ternary_plot to accept SE as input. Plenty of room for improvement on this function. I still think we should include it because it's used in a lot of papers.
