...
libID_contribution <- function(your_data, top_clones_choice, months = c(1:ncol(your_data)), n_clones = 20,
  ...
  rownames(your_data)[nrow(your_data)] <- "NONTOP"
  your_data <- reshape2::melt(as.matrix(your_data))
  colnames(your_data) <- c("BARCODE", "MONTH", "PROP")
  your_data$LIBRARY <- substring(your_data$BARCODE, 1, 6)
  myColors <- c("red", "yellow", "blue", "grey")
  names(myColors) <- c(libIDs, "NONTOP")
