...
#'@param kmeans_cluster Logical. If set to true, each sample will be assigned a cluster computed by kmeans on the reduced dimensional space computed on the measures of dissimilarity between samples.
...
mds_plot = function(your_SE,
  ...
  if (kmeans_cluster){
    plotting_data$kmeans_cluster <- as.factor(kmeans(plotting_data[,c("MDS_1","MDS_2")], k.param)$cluster)
    if (draw_ellipses){
  ...
  if (kmeans_cluster & draw_ellipses){
    p <- p + ggplot2::stat_ellipse(aes(x = MDS_1, y = MDS_2, group = kmeans_cluster), color = "black", linetype = 2)
  }
