...
#'@param capture_efficiency Numeric. The capture efficiency of the clonal tracking method to detect a given clone. Must be between 0 and 1. See the description for details on how to estimate this value for a given experiment. A reasonable value for viral integration site analysis is 0.05 and for DNA barcode sequencing is 0.4 - see Adair, et al 2020. PMID: 32355868.
...
#'@param proportion_labeled Numeric. The proportion of the population which is expected to be genetically modified or contain a clonal tracking index. This is an experimentally determined value.
...
#'@param verbose Logical. Whether to print the calculated thresholds.  
#'
#'@return Returns a list with the element `relative_threshold` describing the proportional abundance above which clones can be considered reliable and the element `absolute_threshold` describing the absolute count above which clones can be considered reliable. Pass either of these values into the function `threshold_SE` to threshold an existing SummarizedExperiment object or the function `create_SE` to threshold a SummarizedExperiment object upon creation from dataframes of counts and metadata.
#'
#'@import SummarizedExperiment
#'@importFrom rlang %||%
#'
#'@examples
#'estimate_barcode_threshold(your_SE = wu_subset, capture_efficiency = 0.4, population_size = 500000, proportion_labeled = 0.3, confidence_level = 0.95, verbose = TRUE)
#'
#'@export
#'
estimate_barcode_threshold <- function(your_SE,
                                       capture_efficiency = NULL,
                                       population_size = NULL,
                                       proportion_labeled = NULL,
  ...
  if (capture_efficiency){
    ...
    cat("No capture efficiency provided. Using 0.05 as default. The capture efficiency depends on the clonal tracking technology and should be estimated for each experiment. \n")
  }
  ...
  # Perform calculation
  output_list <- list()
  output_list[["relative_threshold"]] <- log((1 - confidence_level), base = (1 - capture_efficiency)) / (population_size * proportion_labeled)
  output_list[["absolute_threshold"]] <- log((1 - confidence_level), base = (1 - capture_efficiency)) * capture_efficiency
