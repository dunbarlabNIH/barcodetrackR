...
$logbias=log(\frac{count_1}{count_2}+1)$
...
For this dataset, the weighted heat map shows a more balanced (less biased) clonality between B and T cells as compared to the regular ridge plot. This means that although there are highly biased clones, the largerr clones tend to be shared between B and T cells.

Creating a ridge plot based on the Six et al data also shows that the clonality between B and T cells (as assessed by viral integration site analysis) tends to become less biased over time.
