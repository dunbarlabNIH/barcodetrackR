...
#'@param percent_scale Logical. Whether to display plot with percent scale rather than log scale.
...
BC_clusters <- function(your_data,
  ...
  if(clusters <= 0){
    stop("Must specify at least 1 cluster to be plotted")
  }
  ...
  if(percent_scale){
    myBreaks = pretty(0:100*ceiling(max(melted_data$PROP)))/100
  } else {
    myBreaks = pretty(ourZero, ceiling(max(melted_data$PROP)))
  }
  ...
  ggplot2::ggplot(melted_data, ggplot2::aes(x = SAMPLE, y = PROP, group = COLOR))+
    ...
    ggplot2::scale_y_continuous(breaks = myBreaks)+
