...
system.file("sample_data/KoelleSJ/ZH33_metadata.txt", package = "barcodetrackR") %>%
...
In some cases, we may be interested in whether each barcode changed in proportion from one sample to another. The function barcode_ggheatmap_stat allows users to layer information from statistical tests onto the heat map. Note that this test requires an additional piece of information, which is the sample size of cells for which the barcoding data approximates clonality. In this case, we view the CD16+ NK cells from the Wu dataset at various timepoints, using a sample size of 10,000 barcoded cells.

The stars in the heat map indicate which barcodes in a given sample had a statistically signficant change in proportion, as compared to the previous sample using a chi-squared test. 

```{r barcode_ggheatmap_stat wu_SE, echo=TRUE, warning = FALSE, fig.width = 12, fig.height=10}
...
We again view a comparison in the Wu dataset, this time comparing NK CD56- CD16+ cells to Granulocytes.
...
```{r bias_distribution_barplot wu_SE, echo=TRUE, warning = FALSE, eval = FALSE}
...
Right now, error in line 52 that I can't figure out.
...
```{r ternary_plot wu_SE, echo=TRUE, warning = FALSE, eval = FALSE}
wu_ternary_plot_sample_selection <- c("ZJ31_6m_Gr", "ZJ31_6m_B", "ZJ31_6m_T")
ternary_plot(your_SE = wu_SE[,wu_ternary_plot_sample_selection])
```

A ternary plot for the first three samples from the Six dataset.

```{r ternary_plot six_SE, echo=TRUE, warning = FALSE, eval = FALSE}
ternary_plot(six_SE[,1:3])
```
